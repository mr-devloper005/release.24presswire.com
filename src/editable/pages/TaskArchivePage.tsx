  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#090a0c] text-white">
        <section className="premium-grid border-b border-white/10 bg-[#0d0e11]">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#ffae56]">Distribution desk</p>
            ...
            <p className="max-w-md border-l-4 border-[#ff6a1c] pl-5 text-sm font-bold leading-7 text-white/55">
              Timely reporting, sharp perspectives, and media-ready stories organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-white/10 bg-[#121317] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#ff6a1c]' : 'hover:text-[#ffae56]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#ff6a1c]' : 'whitespace-nowrap hover:text-[#ffae56]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-white/10 bg-[#101114] lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-black lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[#ff6a1c] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
            ...
            <div className="grid">
              <div className="border-b border-white/10 bg-[#ff6a1c] p-6 text-white">
              ...
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-white/10 bg-[#121317] last:border-b-0">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover grayscale transition group-hover:grayscale-0" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#ffae56]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-white/10 bg-[#0d0e11] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-black pb-4">
            ...
            <form action={basePath} className="flex border border-white/15 bg-[#17181c]">
              ...
              <button className="h-11 bg-[#ff6a1c] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-2xl border border-white/10 bg-[#15161a] transition hover:-translate-y-1 hover:border-[#ff6a1c]/60">
                  <div className="aspect-[16/10] overflow-hidden bg-black">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[#ffae56]">
                    ...
                    <p className="mt-4 line-clamp-3 text-sm leading-6 text-white/50">{getSummary(post)}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            ...
            <span className="border-y border-white/15 bg-[#ff6a1c] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
